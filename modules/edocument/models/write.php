                ...
                if (!$index) {
                    ...
                    if (empty($ret)) {
                        if (empty($reciever)) {
                            ...
                            $ret['ret_department'] = Language::replace('Please select :name at least one item', array(':name' => Language::get('Department')));
                        } else {
                            // อัปโหลดไฟล์
                        ...
                        if (empty($ret)) {
                            ...
                            if ($request->post('send_mail')->toInt() == 1) {
                                $err = \Edocument\Email\Model::send(array_keys($recievers), $department, $save);
                                if ($err == '') {
